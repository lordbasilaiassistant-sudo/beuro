// ============================================================
// GrokBok clone — /api/threads
// POST: create a thread (1 bot = DM, 2+ bots = group chat)
// DELETE ?id=: delete a thread (messages cascade)
// ============================================================
...
export async function POST(req: Request) {
  try {
    ...
    const bots = await db.bot.findMany({ where: { id: { in: botIds } } })
    ...
    const thread = await db.thread.create({
      data: { title, isGroup, botIds: JSON.stringify(botIds) },
      include: { messages: true },
    })
    return NextResponse.json({ thread: toThread(thread) })
  } catch (error) {
